    ...
    public static class ClassDefinitions
    {
        ...
        private static StartingClass Assassin()
        {
            return new StartingClass(
                ...
                abilities: new List<ClassAbility>
                {
                    new ClassAbility("Death from Behind", "You have spent a lifetime studying anatomy \u2014 not from books, but from practice. You know exactly where the spine meets the skull, where the arteries run closest to the surface, and where a single precise strike ends a fight before it begins. Attacks that land on an enemy's unguarded back deal significantly increased backstab damage and inflict a powerful stagger that leaves the target reeling. The bonus scales with your Sneak skill, growing deadlier as you refine your approach, and only applies when the enemy is facing away from you.", true, 0),
                    new ClassAbility("Shadow Step", "Your feet remember the discipline of years spent practicing silent movement across creaking floorboards and frozen leaves. While sneaking, you move noticeably faster than other warriors would dare, covering ground at a pace that seems impossible for someone making so little noise. The stamina drain of maintaining stealth is significantly reduced, allowing you to remain hidden for extended periods while repositioning for the perfect strike. Combined with your natural backstab prowess, this makes you a ghost on the battlefield.", false, 10),
                    new ClassAbility("Poison Blade", "During your years in the shadows, you learned the art of coating a blade with toxins extracted from plants, fungi, and creatures that most people avoid entirely. Your backstab attacks now apply a lingering poison that eats away at the target's vitality over several seconds, dealing consistent damage ticks based on your weapon's power. The venom is particularly effective against larger, slower enemies who cannot shake the toxin before it runs its course. Even targets that survive your initial strike will find themselves weakened and vulnerable to follow-up attacks.", false, 25),
                    new ClassAbility("Marked by Fate", "Your assassin's instincts allow you to designate targets with a supernatural awareness that borders on precognition. You can mark up to three enemies simultaneously, causing them to glow faintly in your vision and appear as tracked icons on your minimap regardless of distance or obstacles. Marked targets remain visible even when they flee, hide behind terrain, or burrow underground. The marks persist until the targets are killed or the duration expires, ensuring that no quarry escapes your patient, methodical pursuit.", false, 50)
                }
            );
        }
